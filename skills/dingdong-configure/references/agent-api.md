...
| `dingdong_confirm_mcp_use` | `id`, `serverName`, `toolName` | Re-checks an active MCP after a real tool call and returns the marked replacement footer item; the marker means called, not succeeded |
...
- **MCP:** the bridge returns summary metadata plus the managed `serverName` and, for Codex, `toolNamePrefix`; the enabled server is synchronized to native client configuration. Call its tools only when needed. After a real tool call reaches a terminal result, call `dingdong_confirm_mcp_use` once for that MCP and merge its replacement item. Do not interpret availability or discovery as use evidence.

The Bridge conversation capsule exposes typed items and a light/dark palette: Prompt uses warm orange, Skill blue, and MCP green. Keep the capsule until the final response whenever `conversation.visible` is true. A successful visible `dingdong_load_skill` response returns a replacement `conversation.item` with the same opaque `mergeKey` as its candidate, `confirmedUse: true`, `usage: loaded`, and `marker: "*"`. After a real MCP call reaches any terminal result, `dingdong_confirm_mcp_use` returns the equivalent replacement with `usage: called`; replace only the item with that merge key. Never call it for availability or discovery. A Skill marker means loaded, while an MCP marker means called and does not imply success. Prompt items remain unmarked because delivery cannot prove semantic compliance. Codex desktop includes the current merged footer exactly once as a single Markdown text line, using the exact returned item `lineToken` values and user-configurable symbols. The initial `conversation.line` is final only if no item was replaced. Never display merge keys, resource content, descriptions, IDs, server names, or tool names. Do not use an image, HTML/XML, inline font, or rendering tool for the footer. Explicitly ANSI-capable terminals use current merged ANSI tokens; every other host uses current merged plain-text tokens.

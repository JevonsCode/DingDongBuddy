...
6. 接入后必须区分三类资源的运行语义，不能把 Skill 或 MCP 候选当成 Prompt 执行：
   ...
   - 会话尾部：把 Bridge 返回的 conversation.capsule 作为权威结构化数据保留到最终回复；每次 Skill 加载成功后，只用返回的 conversation.item 替换 mergeKey 相同的候选项。mergeKey 仅用于合并，绝不能展示。只有本轮 dingdong_load_skill 成功返回完整 Skill 且 confirmedUse 为 true 时，才在对应 Skill 名称后加 *，候选 Skill 不加，* 也不代表已经逐条验证 Agent 的执行结果。按能力而不是客户端名称选择显示方式：支持 MCP Apps 时，用最终合并后的 capsule 调用一次 dingdong_render_conversation_footer；明确支持 ANSI 的终端使用 conversation.presentations.ansi.line；其他客户端只显示一次 conversation.line。不要把 MCP App 的 HTML 或 XML 直接贴进普通回复；Prompt、Skill、MCP 分别使用暖橙、蓝、绿

...
6. 接入后必须区分三类资源的运行语义，不能把 Skill 或 MCP 候选当成 Prompt 执行：
   ...
   - 会话尾部：当 conversation.visible 为 true 时，把 Bridge 返回的 conversation.capsule 作为权威结构化数据保留到最终回复；每次 Skill 加载成功后，只用返回的 conversation.item 替换 mergeKey 相同的候选项。mergeKey 仅用于合并，绝不能展示。只有本轮 dingdong_load_skill 成功返回完整 Skill 且 confirmedUse 为 true 时，才在对应 Skill 名称后加 *，候选 Skill 不加，* 也不代表已经逐条验证 Agent 的执行结果。Codex 桌面版只显示一次 conversation.line，作为严格单行的 Markdown 文本；严格使用 conversation.line 与各 item.lineToken 已返回的 Prompt、Skill、MCP 符号，这些符号可由用户配置，不能自行推断或重建。conversation.capsule 的语义配色仍为 Prompt 暖橙、Skill 蓝、MCP 绿。不要在会话尾部使用图片、HTML/XML、行内字体或渲染工具。仅在明确支持 ANSI 的终端使用 conversation.presentations.ansi.line，其他客户端只显示一次 conversation.fallbackLine

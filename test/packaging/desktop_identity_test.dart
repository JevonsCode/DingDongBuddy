...
void main() {
  ...
  test('desktop hosts consume application version 1.5.5 from pubspec', () {
    ...
    expect(pubspec, contains('version: 1.5.5+60'));
    expect(
      releaseVersion,
      contains("const String currentAppVersion = '1.5.5';"),
    );
    expect(releaseVersion, contains("const String currentAppBuild = '60';"));
    expect(
      File('lib/features/agent_api/data/mcp_server.dart').readAsStringSync(),
      contains("'version': '1.5.5'"),
    );
    expect(
      File(
    ...
    expect(windowsResources, contains('#define VERSION_AS_NUMBER 1,5,5,60'));
    expect(windowsResources, contains('#define VERSION_AS_STRING "1.5.5"'));
  });
  ...
  test('website keeps release diagnostics behind debug mode', () {
    ...
    expect(website, contains('<span class="demo-version">v1.5.5</span>'));
    ...
    expect(releaseMetadata, contains('"latestVersion": "1.5.5"'));
    expect(releaseMetadata, contains('"latestBuild": "60"'));
    expect(releaseMetadata, contains('"prerelease": false'));
    expect(
      releaseMetadata,
      contains('Refines macOS tray unread badges'),
    );
    expect(
      releaseMetadata,
      contains('Fixes stuck shortcut hints after releasing Ctrl'),
    );
    expect(releaseMetadata, contains('"arm64"'));
    expect(releaseMetadata, contains('"x86_64"'));
    expect(releaseMetadata, contains('"beta": false'));
    expect(releaseMetadata, contains('DingDong-1.5.5-windows-x64-Setup.exe'));
  });

...
test("pairing never promises or displays unsent host history", () => {
  ...
  assert.match(serviceWorkerSource, /dingdong-app-shell-v29/);
});
...
test("PWA settings can check and apply an update without replacing pairings", () => {
  ...
  assert.match(appSource, /const currentPwaVersion = "1\.5\.1"/);
  assert.match(appSource, /const currentPwaShellVersion = 29/);
  ...
  assert.deepEqual(pwaVersion, { version: "1.5.1", shell: 29 });
});

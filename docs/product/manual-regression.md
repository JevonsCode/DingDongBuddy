# DingDong 1.5.1 Manual Regression Checklist
...
- The macOS release app metadata is version `1.5.1` build `56` and bundle id `com.dingdongbuddy.app`.
- The Windows executable metadata is version `1.5.1.56` and product name `DingDong`.
...
- Deploy the device-link Worker from the tested `main` commit either through a
  ...
  `/v1/health` must report version `1.5.1` and that exact commit SHA; every

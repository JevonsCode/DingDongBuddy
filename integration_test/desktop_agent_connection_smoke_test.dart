...
void main() {
  ...
  testWidgets(
    'real selection host obeys its switch and permission boundary',
    (WidgetTester tester) async {
      await _showTestWindow();
      const NativeSelectionPluginGateway gateway =
          NativeSelectionPluginGateway();
      // Exercise the actual macOS channel and controller without user preferences,
      // Keychain tokens, network requests, or automatic permission prompts.
      ...
      await model.load();
      final bool permissionGranted = (await gateway.status()).permissionGranted;
      ...
      expect((await gateway.status()).running, isFalse);

      await tester.tap(find.byKey(const Key('settings-selection-enabled')));
      await tester.pumpAndSettle();
      final status = await gateway.status();
      expect(status.enabled, isTrue);
      expect(status.permissionGranted, permissionGranted);
      if (!permissionGranted) expect(status.running, isFalse);
      expect(model.isSelectionPluginRunning, status.running);

      await tester.tap(find.byKey(const Key('settings-selection-enabled')));
      await tester.pumpAndSettle();
      expect((await gateway.status()).enabled, isFalse);
      expect((await gateway.status()).running, isFalse);
      expect((await gateway.status()).permissionGranted, permissionGranted);
    },
  ...
  testWidgets(
    'real loopback fallback is healthy and shown consistently',
    (WidgetTester tester) async {
      await _showTestWindow();
      ...
      expect(
        find.textContaining(
          'API listening on 127.0.0.1:${actualEndpoint.port}',
        ),
        findsOneWidget,
      );
...
Future<void> _showTestWindow() async {
  // The menu-bar host starts hidden. Show it before pumpWidget: the very first
  // pump can wait indefinitely for a frame from a hidden macOS window.

...
在本机 Codex、Claude Code、Cursor 或 Gemini CLI 会话中粘贴下面这句话即可。Agent
...
- 把已启用、全局、始终生效的 Prompt 同步到 Codex 的
  `~/.codex/AGENTS.md` 托管区块，同时保留原有用户规则
...
- 页头在 **DingDong** 右侧显示当前应用版本，例如 `v0.7.20`；版本值和应用的
  发布版本共用同一个常量。点击版本号会打开设置，并直接定位到版本与更新区域。
...
- “最近 Agent”标题旁显示克制的滚动时间窗口计数，默认统计近 24 小时；完成详情默认
  保留最近 200 条并跨重启记忆，也可在资源管理中以只读列表查看。是否跨重启保留、
  详情上限和计数小时数都可在设置中调整。
...
这两条接入链路和用户在 DingDong 里启用的资源是两回事。Codex 的全局、始终生效
Prompt 会写入 `~/.codex/AGENTS.md` 的 DingDong 托管区块；带项目规则或任务匹配的
Prompt 由 `dingdong_bridge` 路由，Manual Prompt 不会自动激活。已启用的 Skill 会
作为完整目录同步到客户端原生 Skill 位置；已启用的 MCP 会写成真实的客户端 MCP
配置。Bridge 默认返回 Prompt 完整正文，而 Skill 和 MCP 保持摘要优先。

### Prompt、Skill 和 MCP 的调用逻辑

| 类型 | 如何进入 Agent | Agent 应如何处理 |
|---|---|---|
| Prompt | Codex 的全局、始终生效 Prompt 直接写入 DingDong 托管的 `AGENTS.md`；项目或任务 Prompt 由 Bridge 返回完整正文 | 每个命中的 Prompt 都是必须自动应用的指令，不需要 Agent 再决定是否调用 |
| Skill | 已启用 Skill 同步成客户端原生 Skill Package；Bridge 只返回候选摘要 | 先按 description 判断是否匹配，匹配任务时才加载或使用完整 Skill；摘要本身不是指令 |
| MCP | 已启用 MCP 写入客户端原生 MCP 配置；Bridge 只返回候选摘要 | 配置只表示工具可用，任务确实需要时才调用对应工具；不要求每轮调用 |

激活方式和触发组控制 Bridge 的候选路由。当前版本仍会把已启用的原生 Skill 和 MCP
全局同步给客户端，因此“客户端可见”和“本轮实际加载或调用”是两件事。
...
flowchart TB
  ...
  subgraph NativeConfig["Agent 用户级原生配置"]
    McpConfig["MCP 配置<br/>Codex TOML · Claude/Cursor/Gemini JSON"]
    HookConfig["完成 Hook<br/>Stop · afterAgentResponse · AfterAgent"]
    PromptFile["Codex 全局 Prompt<br/>~/.codex/AGENTS.md 托管区块"]
  ...
  subgraph Sync["原生资源同步"]
    ...
    PromptWriter["Codex Prompt 托管写入<br/>保留用户原有规则"]
...
- **启用资源**：资源库启用状态 → Codex 全局 Prompt 托管区块、原生 Skill 目录
  或 MCP 配置；DingDong 使用托管标记避免误删用户文件。项目规则在
  `dingdong_bridge` 为任务路由资源时单独生效。
...
5. 保持三类资源语义独立：Prompt 命中后完整、自动应用；Skill 先按 description 匹配再按需加载；MCP 只在任务需要时调用工具。Skill/MCP 摘要都不是 Prompt 指令。

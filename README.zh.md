...
- 页头在 **DingDong** 右侧显示当前应用版本，例如 `v0.9.2`；版本值和应用的
...
系统级的 **打开或隐藏剪贴板** 快捷键可在 **设置 → 键盘快捷键** 中修改。点击当前
快捷键后直接按下新组合，录入后立即生效。组合必须至少包含一个修饰键，主键可使用
字母、数字、F1–F12、方向键、空格或回车。如果该组合已被其他应用占用，DingDong
会继续使用原来的快捷键。

| 操作 | macOS | Windows |
| --- | --- | --- |
| 打开或隐藏剪贴板 | `⌘⇧V`（可配置） | `Ctrl+Shift+V`（可配置） |
| 打开动态 / 资源库 / 剪贴板 | `⌘Q` / `⌘W` / `⌘E` | `Ctrl+Q` / `Ctrl+W` / `Ctrl+E` |
| 聚焦剪贴板搜索 | `⌘F` | `Ctrl+F` |
| 展开或收起剪贴板筛选 | `⌘R` | `Ctrl+R` |
